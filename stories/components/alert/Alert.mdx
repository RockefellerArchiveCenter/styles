...
- Use the `role='alert'` or `role='status'` attribute to announce alerts with screen readers. Use
'alert' for urgent messages that will interrupt screen readers, and 'status' for messages
that can wait for the screen reader to finish its current task before being announced. To implement
dynamically without a page reload, there must be an existing hidden live region with the aria role
in the HTML in which the alert contents is inserted. The screen reader will announce dynamic changes
to the content in this region.
...
- Alerts should not disappear automatically. If you want users to be able to remove
  alerts, create a dismissable alert with a close button.
...
- Do not use a notification banner to tell the user about validation errors.
  Instead, use an error message and error summary.
